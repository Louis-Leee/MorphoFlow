"""
Cross-Embodiment Diffusion V3 training script with PyTorch Lightning + Hydra.

Uses RobotGraphV3CE with mixed GT/no-GT dataset and classifier-free guidance.
GT robots train with object-conditioned diffusion; no-GT robots train with
self-reconstruction of valid FK poses (skip_or=True).

Usage:
    # Single GPU
    python train_diff_v3_ce.py

    # Multi-GPU DDP
    python train_diff_v3_ce.py train.gpus=4

    # Override config via Hydra CLI
    python train_diff_v3_ce.py train.lr=2e-4 dataset.batch_size=32

    # Resume from checkpoint
    python train_diff_v3_ce.py train.resume_from=graph_exp/diff_v3_ce/ckpt/epoch=49.ckpt
"""
...
from model.tro_graph_v3_ce import RobotGraphV3CE
from dataset.CrossEmbodimentDataset import CrossEmbodimentDataset, ce_custom_collate_fn


class DiffusionV3CEModule(pl.LightningModule):
    """Lightning module wrapping RobotGraphV3CE for DDP training."""

    def __init__(self, model_cfg: dict, train_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.model = RobotGraphV3CE(**model_cfg)
...
class CEGraspDataModule(pl.LightningDataModule):
    """DataModule wrapping CrossEmbodimentDataset for DDP-compatible training."""
...
@hydra.main(config_path="config", config_name="train_diff_v3_ce", version_base=None)
def main(cfg: DictConfig):
    print("\n" + "=" * 60)
    print("CROSS-EMBODIMENT DIFFUSION V3 TRAINING")
    print("=" * 60)
    ...
    module = DiffusionV3CEModule(model_cfg, train_cfg)
    ...
    datamodule = CEGraspDataModule(cfg.dataset)
