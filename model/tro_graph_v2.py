"""
RobotGraphV2: Diffusion model using FlashAttentionDenoiser.

Based on tro_graph.py (RobotGraph) with minimal changes:
1. FlashAttentionDenoiser replaces GraphDenoiser
2. Self-loop edges zeroed out in noisy E_RR (original masks were dead code)
...
class RobotGraphV2(nn.Module):
    ...
    def __init__(
        ...
        super(RobotGraphV2, self).__init__()
    ...
    def _build_noisy_edges(self, noisy_V_R, V_O, device, dtype):
        """Build noisy graph edges.

        Self-loops are kept (matching original tro_graph.py behavior)
        to ensure a fair comparison isolating only the denoiser change.
        """
    ...
    # ------------------------------------------------------------------
    # Forward: Diffusion training (identical logic to tro_graph.py)
    # ------------------------------------------------------------------
    def forward(self, batch, eps=1e-8):
        ...
        # ---- Build noisy edges (self-loops zeroed) ----
        ...
        pred_link_noise = self.denoiser(
            V_O, noisy_V_R, noisy_E_OR, noisy_E_RR, t
        )
    ...
    # ------------------------------------------------------------------
    # Inference: DDIM sampling (identical logic to tro_graph.py)
    # ------------------------------------------------------------------
    @torch.no_grad()
