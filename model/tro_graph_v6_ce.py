"""
RobotGraphV6CE: Cross-Embodiment Diffusion V6 with SE3 Relative Position Bias.

Key differences from V4 (RobotGraphV4CE):
1. SE3 RPB: Relative SE3 transforms between robot links (E_RR) and
   between robot links and object patches (E_OR) are projected to
   per-head attention bias. This gives the transformer geometric
   inductive bias without graph structure.
2. BPS/VAE link feature switch: Supports both BPS (V3-style) and TripoSG VAE
   (V4-style) link embeddings via config param `link_feature_type`.
3. 8-layer denoiser (increased from 6) for more model capacity.

Uses FlashAttentionDenoiser from model/flash_denoiser.py.
Uses CrossEmbodimentDatasetV5 (dual object split + overlap) for training.
"""
...
from model.flash_denoiser import FlashAttentionDenoiser
from utils.hand_model import create_hand_model

ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))


class RobotGraphV6CE(nn.Module):
    ...
    def __init__(
        ...
        super(RobotGraphV6CE, self).__init__()
        ...
        self.link_feature_type = link_feature_type
        self.hand_dict = {}
        pc_type = '512_uniform' if link_feature_type == 'vae' else None
        for hand_name in self.embodiment:
            self.hand_dict[hand_name] = create_hand_model(hand_name, pc_type=pc_type)

        self.object_patch = object_patch
        self.max_link_node = max_link_node
        self.robot_links = robot_links

        # ---- Link embedding: BPS or VAE ----
        if link_feature_type == 'vae':
            if isinstance(vae_config, dict):
                vae_config = OmegaConf.create(vae_config)
            self._vae_config = vae_config
            self.vae_latent_dim = vae_config.get("latent_channels", 64)
            self.link_embed_dim = vae_config.get("link_embed_dim", 128)
            self.link_token_encoder = nn.Sequential(
                nn.Linear(self.vae_latent_dim, self.link_embed_dim),
                nn.ReLU(inplace=True),
                nn.Linear(self.link_embed_dim, self.link_embed_dim),
            )
        elif link_feature_type == 'bps':
            if isinstance(bps_config, dict):
                bps_config = OmegaConf.create(bps_config)
            from bps_torch.bps import bps_torch
            self.link_embed_dim = bps_config.n_bps_points + 4  # 124 + 4 = 128
            ...
            raise ValueError(f"Unknown link_feature_type: {link_feature_type}")
        ...
        # ---- Build and register link embeddings ----
        if link_feature_type == 'vae':
            self._register_link_embeddings(self._construct_vae_embeddings())
        elif link_feature_type == 'bps':
            self.link_embeddings = self._construct_bps()
    ...
    # ------------------------------------------------------------------
    # Link embedding: TripoSG VAE (same as V4)
    # ------------------------------------------------------------------
    def _construct_vae_embeddings(self):
        """Encode per-link point clouds with frozen TripoSG VAE encoder."""
        from model.autoencoders.triposg_varlen_autoencoder import TripoSGVarlenVAEModel

        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        print("Loading TripoSG VAE encoder for link embedding computation...")
        vae = TripoSGVarlenVAEModel.from_pretrained(
            self._vae_config.pretrained_model,
            subfolder=self._vae_config.subfolder,
        ...
        for embodiment, hand_model in self.hand_dict.items():
            ...
            for ln in link_names:
                pc = hand_model.links_pc[ln]
                normal = hand_model.links_pc_normal[ln]
                ...
                centroid, scale = self._unit_ball_(pc)
                pc = (pc - centroid) / scale
                link_inputs.append(torch.cat([pc, normal], dim=-1))

            batch = torch.stack(link_inputs, dim=0)
            flat = batch.reshape(-1, 6).to(device, dtype=torch.float16)

            with torch.no_grad():
                _, posterior = vae.encode_shape(
                    flat, num_tokens=1, batch_size=len(link_names)
                )
    ...
    # ------------------------------------------------------------------
    # Link embedding: BPS (same as V3)
    # ------------------------------------------------------------------
    def _construct_bps(self):
        """Encode per-link point clouds with BPS distances."""
        link_embedding_dict = {}
        for embodiment, hand_model in self.hand_dict.items():
    ...
    # ------------------------------------------------------------------
    # Link embedding buffer management (VAE mode)
    # ------------------------------------------------------------------
    def _register_link_embeddings(self, link_embedding_dict):
        """Register link embeddings as persistent buffers for checkpoint save/load."""
        self._link_emb_names = list(link_embedding_dict.keys())
        for name, emb in link_embedding_dict.items():
            self.register_buffer(f'_link_emb_{name}', emb, persistent=True)

    @property
    def link_embeddings(self):
        if self.link_feature_type == 'vae':
            return {name: getattr(self, f'_link_emb_{name}') for name in self._link_emb_names}
        else:
            # BPS mode: link_embeddings is a plain dict set in __init__
            return self._bps_link_embeddings

    @link_embeddings.setter
    def link_embeddings(self, value):
        """Setter for BPS mode where embeddings are stored as plain dict."""
        self._bps_link_embeddings = value
    ...
    # ------------------------------------------------------------------
    def _compute_edges(self, noisy_V_R, V_O_or_positions, device, dtype):
        """Compute relative SE3 edges from noisy robot poses and object positions.

        Args:
            noisy_V_R: [B, L, D] noisy robot link features (first 6 dims = SE3 pose)
            V_O_or_positions: [B, P, D] object node features (first 3 dims = positions)
            device: torch device
            dtype: torch dtype

        Returns:
            noisy_E_RR: [B, L, L, 6] robot-robot relative SE3 edges
            noisy_E_OR: [B, L, P, 6] robot-object relative SE3 edges
        """
        B_T = noisy_V_R.shape[0]
        L = noisy_V_R.shape[1]

        # Robot poses -> SE3 matrices
        noisy_V_R_se3 = vector_to_matrix(noisy_V_R[:, :, :6])  # [B_T, L, 4, 4]

        # RR edges: pairwise relative SE3 between robot links
        noisy_E_RR = matrix_to_vector(
            compute_batch_relative_se3(noisy_V_R_se3, noisy_V_R_se3)
        )  # [B_T, L, L, 6]

        # Object positions -> SE3 matrices (identity rotation + translation)
        object_positions = V_O_or_positions[:, :, :3]
        P = object_positions.shape[1]
        object_se3 = torch.eye(4, device=device, dtype=dtype).expand(B_T, P, -1, -1).clone()
        object_se3[:, :, :3, 3] = object_positions

        # OR edges: relative SE3 between robot links and object patches
        noisy_E_OR = matrix_to_vector(
            compute_batch_relative_se3(noisy_V_R_se3, object_se3)
        )  # [B_T, L, P, 6]

        return noisy_E_RR, noisy_E_OR
    ...
    def _forward_gt(self, batch, eps=1e-8):
        ...
        # ---- Compute noisy relative SE3 edges ----
        noisy_E_RR, noisy_E_OR = self._compute_edges(noisy_V_R, V_O, device, dtype)

        # ---- Predict noise ----
        pred_link_noise = self.denoiser(
            V_O, noisy_V_R, noisy_E_OR, noisy_E_RR, t, skip_or=use_uncond
        )
    ...
    def _forward_nogt(self, batch, eps=1e-8):
        ...
        if has_obj_norm:
            centroids = batch["object_centroids"].to(device)
            scales = batch["object_scales"].to(device)
        ...
        for b in range(B):
            ...
            if has_obj_norm:
                object_center = centroids[b].unsqueeze(0)
                object_scale = scales[b].unsqueeze(0)
                target_trans = (target_trans - object_center) / object_scale
            else:
    ...
    # ------------------------------------------------------------------
    # Inference: DDIM sampling with classifier-free guidance + edges
    # ------------------------------------------------------------------
    @torch.no_grad()
    ...
    def _inference_unconditioned(
        ...
        for i, diffuse_step in enumerate(ddim_t):
            ...
            # ---- Compute edges from current noisy poses ----
            noisy_E_RR, noisy_E_OR = self._compute_edges(
                noisy_V_R, object_nodes, device, dtype
            )
            noisy_E_RR_dummy, noisy_E_OR_dummy = self._compute_edges(
                noisy_V_R, dummy_object_nodes, device, dtype
            )

            # ---- CFG: two passes with edges ----
            if self.guidance_scale != 1.0:
                pred_uncond = self.denoiser(
                    dummy_object_nodes, noisy_V_R,
                    noisy_E_OR_dummy, noisy_E_RR,
                    t_batch, skip_or=True,
                )
                pred_cond = self.denoiser(
                    object_nodes, noisy_V_R,
                    noisy_E_OR, noisy_E_RR,
                    t_batch, skip_or=False,
                )
