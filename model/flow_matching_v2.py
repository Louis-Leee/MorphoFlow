"""
FlowMatchingV2: SE(3) flow matching with FlashAttentionDenoiser.

Based on FlowMatchingRobotGraph with key improvements:
1. Uses FlashAttentionDenoiser (standard transformer) instead of GraphDenoiser
2. Excludes self-loop edges in robot-robot graph (consistent with diffusion model)
3. Default ode_steps increased from 50 to 100

Reuses VQVAE from the existing codebase without modification.
"""
...
class FlowMatchingV2(nn.Module):
    ...
    def __init__(
        ...
        super(FlowMatchingV2, self).__init__()
    ...
    def init_flow_matching(self, cfg):
        self.M = cfg["M"]
        self.ode_steps = cfg.get("ode_steps", 100)  # default 100 (up from 50)
        self.solver = cfg.get("solver", "euler")

    # ------------------------------------------------------------------
    # Helper methods (identical to FlowMatchingRobotGraph)
    # ------------------------------------------------------------------
    ...
    def _build_edges(self, robot_nodes, object_nodes, batch, B, device, dtype):
        ...
        Unlike FlowMatchingRobotGraph, self-loop edges are excluded
        (consistent with the diffusion model in tro_graph.py).
        """
        ...
        for b in range(B):
            ...
            # Exclude self-loop edges (diagonal = identity, carries no info)
            link_rel_poses[b, torch.arange(num_link), torch.arange(num_link), :] = 0.0
    ...
    def _build_noisy_edges(self, noisy_V_R, V_O, device, dtype):
        ...
        Self-loop edges in E_RR are zeroed out.
        """
        ...
        # Zero out self-loop edges (diagonal)
        BT, L = noisy_E_RR.shape[0], noisy_E_RR.shape[1]
        idx = torch.arange(L, device=device)
        noisy_E_RR[:, idx, idx, :] = 0.0
    ...
    def _inference_palm_conditioned(
        ...
        V_R_trans = (1.0 - t_start) * x0_trans
