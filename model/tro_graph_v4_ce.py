"""
RobotGraphV4CE: Cross-Embodiment Diffusion V4 with TripoSG VAE Link Features.

Based on tro_graph_v3_ce.py (RobotGraphV3CE). The only change is replacing
BPS (Ball Point Set) link embeddings with frozen TripoSG VAE encoder latents:

  V3: link_pc → normalize → BPS distances → [num_links, 128]
  V4: link_pc + normal → TripoSG VAE encode_shape(num_tokens=1) → [num_links, 64]
      → Linear(64, 128) to match downstream denoiser V_robot_dims=[3, 3, 128]

Everything else (diffusion, denoiser, loss, inference) is identical to V3.
"""
...
class RobotGraphV4CE(nn.Module):
    ...
    def __init__(
        ...
        for hand_name in self.embodiment:
            self.hand_dict[hand_name] = create_hand_model(hand_name, pc_type='512_uniform')

        self.object_patch = object_patch
        self.max_link_node = max_link_node

        # ---- Link embedding (TripoSG VAE) ----
        ...
        self.mode = mode
        self._register_link_embeddings(self.construct_vae())
    ...
    def construct_vae(self):
        ...
        Uses hand_model.links_pc (filtered) and hand_model.links_pc_normal
        loaded from data/PointCloud_512_uniform/robot/{name}.pt via
        create_hand_model(pc_type='512_uniform'). Encodes each link with
        encode_shape(num_tokens=1) to get a 64D VAE latent per link.

        Returns:
            dict: {embodiment_name: Tensor[num_links, vae_latent_dim]}
        """
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        # Load frozen TripoSG encoder temporarily (fp16 required for flash_attn)
        ...
        for embodiment, hand_model in self.hand_dict.items():
            ...
            for ln in link_names:
                pc = hand_model.links_pc[ln]            # [512, 3]
                normal = hand_model.links_pc_normal[ln]  # [512, 3]
                ...
                link_inputs.append(torch.cat([pc, normal], dim=-1))  # [512, 6]

            batch = torch.stack(link_inputs, dim=0)  # [num_links, 512, 6]
            ...
            with torch.no_grad():
                ...
                latent = posterior.mode().float().cpu()  # [num_links, 64]
    ...
    # ------------------------------------------------------------------
    # Link embedding buffer management
    # ------------------------------------------------------------------
