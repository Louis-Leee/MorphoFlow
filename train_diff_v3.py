"""
Diffusion V3 training script with PyTorch Lightning + Hydra.

Uses RobotGraphV3 (Diffusion + FlashAttentionDenoiserNoEdge).
No-edge ablation: removes explicit SE(3) edge computation.

Usage:
    # Single GPU
    python train_diff_v3.py

    # Multi-GPU DDP
    python train_diff_v3.py train.gpus=4

    # Override config via Hydra CLI
    python train_diff_v3.py train.lr=2e-4 dataset.batch_size=32

    # Resume from checkpoint
    python train_diff_v3.py train.resume_from=graph_exp/diff_v3/ckpt/epoch=49.ckpt
"""
...
class DiffusionV3Module(pl.LightningModule):
    """Lightning module wrapping RobotGraphV3 for DDP training."""

    def __init__(self, model_cfg: dict, train_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.model = RobotGraphV3(**model_cfg)
...
@hydra.main(config_path="config", config_name="train_diff_v3", version_base=None)
def main(cfg: DictConfig):
    print("\n" + "=" * 60)
    print("DIFFUSION V3 TRAINING (No Edge)")
    print("=" * 60)
    ...
    module = DiffusionV3Module(model_cfg, train_cfg)
