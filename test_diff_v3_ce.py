"""
Cross-Embodiment Diffusion V3 evaluation script.

Uses RobotGraphV3CE with classifier-free guidance at inference.
Supports batch evaluation across multiple robot hands from a single config.

Usage:
    # Single hand (backward compatible)
    python test_diff_v3_ce.py --config config/test_diff_v3_ce.yaml
    ...
    python test_diff_v3_ce.py --config config/test_diff_v3_ce.yaml \
        --hands allegro barrett shadowhand leaphand \
        --ckpt graph_exp/diff_v3_ce/ckpt/epoch=299.ckpt --gpu 0

    # Use JAX on CPU to save GPU memory for IK:
    JAX_PLATFORM_NAME=cpu python test_diff_v3_ce.py --config config/test_diff_v3_ce.yaml
"""
...
def test(config, hand_overrides=None, ckpt_override=None, gpu_override=None):
    ...
    model = RobotGraphV3CE(**config.model).to(device)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Cross-Embodiment Diffusion V3 evaluation (single or batch multi-hand)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config/test_diff_v3_ce.yaml",
