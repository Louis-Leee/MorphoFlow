"""
Cross-Embodiment Diffusion V4 (VAE) training script with PyTorch Lightning + Hydra.

Uses RobotGraphV4CE which replaces BPS link embeddings with frozen TripoSG VAE
encoder latents. Everything else is identical to train_diff_v3_ce.py.

Usage:
    # Single GPU
    python train_diff_v4_ce_vae.py

    # Multi-GPU DDP
    python train_diff_v4_ce_vae.py train.gpus=4

    # Override config via Hydra CLI
    python train_diff_v4_ce_vae.py train.lr=2e-4 dataset.batch_size=32

    # Resume from checkpoint
    python train_diff_v4_ce_vae.py train.resume_from=graph_exp/diff_v4_ce_vae/ckpt/epoch=49.ckpt
"""
...
class DiffusionV4CEVAEModule(pl.LightningModule):
...
@hydra.main(config_path="config", config_name="train_diff_v4_ce_vae", version_base=None)
def main(cfg: DictConfig):
    print("\n" + "=" * 60)
    print("CROSS-EMBODIMENT DIFFUSION V4 (VAE) TRAINING")
    print("=" * 60)
    ...
    module = DiffusionV4CEVAEModule(model_cfg, train_cfg)
