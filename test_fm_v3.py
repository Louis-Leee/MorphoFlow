"""
Flow Matching V3 evaluation script.

Uses FlowMatchingV3 with FlashAttentionDenoiserNoEdge (no edge computation).
Handles both Lightning checkpoint format and raw state_dict.

Usage:
    python test_fm_v3.py --config config/test_fm_v3.yaml
    # Use JAX on CPU to save GPU memory for IK:
    JAX_PLATFORM_NAME=cpu python test_fm_v3.py --config config/test_fm_v3.yaml
"""
...
def test(config):
    ...
    model = FlowMatchingV3(**config.model).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/test_fm_v3.yaml",
        help="config file",
    )
