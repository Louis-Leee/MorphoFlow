"""
Diffusion V2 evaluation script.

Uses RobotGraphV2 with FlashAttentionDenoiser.
Handles both Lightning checkpoint format and raw state_dict.

Usage:
    python test_diff_v2.py --config config/test_diff_v2.yaml
    # Use JAX on CPU to save GPU memory for IK:
    JAX_PLATFORM_NAME=cpu python test_diff_v2.py --config config/test_diff_v2.yaml
"""
...
def test(config):
    ...
    model = RobotGraphV2(**config.model).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/test_diff_v2.yaml",
        help="config file",
    )
