"""
Flow Matching V2 training script with PyTorch Lightning + Hydra.

Uses FlashAttentionDenoiser instead of GraphDenoiser.

Usage:
    # Single GPU
    python train_fm_v2.py

    # Multi-GPU DDP
    python train_fm_v2.py train.gpus=4

    # Override config via Hydra CLI
    python train_fm_v2.py train.lr=2e-4 dataset.batch_size=32

    # Resume from checkpoint
    python train_fm_v2.py train.resume_from=graph_exp/fm_v2/ckpt/epoch=49.ckpt
"""
...
from model.flow_matching_v2 import FlowMatchingV2
from dataset.CMapDataset import CMapDataset, custom_collate_fn


class FlowMatchingV2Module(pl.LightningModule):
    """Lightning module wrapping FlowMatchingV2 for DDP training."""

    def __init__(self, model_cfg: dict, train_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.model = FlowMatchingV2(**model_cfg)
...
@hydra.main(config_path="config", config_name="train_fm_v2", version_base=None)
def main(cfg: DictConfig):
    print("\n" + "=" * 60)
    print("FLOW MATCHING V2 TRAINING (Flash Attention)")
    print("=" * 60)
    ...
    module = FlowMatchingV2Module(model_cfg, train_cfg)
