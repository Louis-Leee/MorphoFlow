"""
Diffusion V2 training script with PyTorch Lightning + Hydra.

Uses RobotGraphV2 (Diffusion + FlashAttentionDenoiser).

Usage:
    # Single GPU
    python train_diff_v2.py

    # Multi-GPU DDP
    python train_diff_v2.py train.gpus=4

    # Override config via Hydra CLI
    python train_diff_v2.py train.lr=2e-4 dataset.batch_size=32

    # Resume from checkpoint
    python train_diff_v2.py train.resume_from=graph_exp/diff_v2/ckpt/epoch=49.ckpt
"""
...
from model.tro_graph_v2 import RobotGraphV2
from dataset.CMapDataset import CMapDataset, custom_collate_fn


class DiffusionV2Module(pl.LightningModule):
    """Lightning module wrapping RobotGraphV2 for DDP training."""

    def __init__(self, model_cfg: dict, train_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.model = RobotGraphV2(**model_cfg)
...
@hydra.main(config_path="config", config_name="train_diff_v2", version_base=None)
def main(cfg: DictConfig):
    print("\n" + "=" * 60)
    print("DIFFUSION V2 TRAINING (Flash Attention)")
    print("=" * 60)
    ...
    module = DiffusionV2Module(model_cfg, train_cfg)
