"""
CrossEmbodimentDatasetV5: Dual-split dataset with GT/no-GT overlap support.

Extends CrossEmbodimentDataset with two key features:
1. Separate object split files for GT vs no-GT robots (no-GT can use a larger
   training set since object features are not used).
2. GT and no-GT robot lists CAN overlap — the same robot can appear in both,
   trained with object conditioning (GT) and without (no-GT) as separate entries.

Each __getitem__ returns a full batch (batch_size samples) for one (robot, mode)
entry. Selection is uniform across all entries.
"""
...
class CrossEmbodimentDatasetV5(Dataset):
    def __init__(
        self,
        batch_size: int,
        gt_robot_names: list,
        ...
        no_gt_split_json_path: str = None,
    ):
        ...
        # Build (robot_name, mode) entries for overlap support
        self.robot_entries = []
        ...
        # Deduplicated list for hand model loading
        self.unique_robot_names = list(dict.fromkeys(
            self.gt_robot_names + self.no_gt_robot_names
        ))

        # Load hand models (deduplicated)
        self.hands = {}
        self.dofs = []
        for robot_name in self.unique_robot_names:
            self.hands[robot_name] = create_hand_model(robot_name, torch.device('cpu'))
            self.dofs.append(math.sqrt(self.hands[robot_name].dof))

        # --- Dual splits ---
        gt_split_path = os.path.join(ROOT_DIR, 'data/CMapDataset_filtered/split_train_validate_objects.json')
        gt_split = json.load(open(gt_split_path))
        self.gt_object_names = gt_split['train'] if is_train else gt_split['validate']

        if no_gt_split_json_path:
            if not os.path.isabs(no_gt_split_json_path):
                no_gt_split_json_path = os.path.join(ROOT_DIR, no_gt_split_json_path)
            nogt_split = json.load(open(no_gt_split_json_path))
            self.nogt_object_names = nogt_split['train'] if is_train else nogt_split['validate']
            print(f"No-GT split: {len(self.nogt_object_names)} objects from {no_gt_split_json_path}")
        else:
            self.nogt_object_names = self.gt_object_names

        if debug_object_names is not None:
            print("!!! Using debug objects !!!")
            self.gt_object_names = debug_object_names
            self.nogt_object_names = debug_object_names

        print(f"GT objects: {len(self.gt_object_names)}, No-GT objects: {len(self.nogt_object_names)}")
        ...
        # GT metadata: filter by gt_object_names AND gt_robot_set
        gt_obj_set = set(self.gt_object_names)
        self.metadata = [m for m in metadata if m[1] in gt_obj_set and m[2] in self.gt_robot_set]

        # Group GT metadata by robot
        self.metadata_by_robot = {}
        for m in self.metadata:
            robot = m[2]
            if robot not in self.metadata_by_robot:
                self.metadata_by_robot[robot] = []
            self.metadata_by_robot[robot].append((m[0], m[1]))

        # No-GT metadata: filter by nogt_object_names AND no_gt_robot_set
        nogt_obj_set = set(self.nogt_object_names)
        no_gt_robot_set = set(self.no_gt_robot_names)
        self.nogt_metadata_by_robot = {}
        for m in metadata:
            if m[2] in no_gt_robot_set and m[1] in nogt_obj_set:
                robot = m[2]
        ...
        # Object point clouds — load for union of both object sets
        all_object_names = list(set(self.gt_object_names) | set(self.nogt_object_names))
        self.object_pcs = {}
        self.object_normals = {}
        ...
        if not is_train:
            # Validation: unique robots × GT objects (validate on held-out split only)
            self.combination = []
            for robot_name in self.unique_robot_names:
                for object_name in self.gt_object_names:
...
def ce_v5_custom_collate_fn(batch):
    return batch[0]


def create_ce_v5_dataloader(cfg, is_train):
    dataset = CrossEmbodimentDatasetV5(
        ...
        no_gt_split_json_path=cfg.get('no_gt_split_json_path', None),
    )
    dataloader = DataLoader(
        dataset,
        batch_size=1,
        collate_fn=ce_v5_custom_collate_fn,
