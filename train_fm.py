"""
Flow Matching training script with PyTorch Lightning + Hydra.
...
Usage:
    ...
    # Multi-GPU DDP
    python train_fm.py train.gpus=4

    # Override config via Hydra CLI
    python train_fm.py train.lr=2e-4 dataset.batch_size=32

    # Resume from checkpoint
    python train_fm.py train.resume_from=graph_exp/fm_bal/ckpt/epoch=49.ckpt

    # Transfer from diffusion weights
    python train_fm.py train.pretrained_from=ckpt/multi_hand.pth
"""
...
class FlowMatchingGraspModule(pl.LightningModule):
    """Lightning module wrapping FlowMatchingRobotGraph for DDP training."""

    def __init__(self, model_cfg: dict, train_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.model = FlowMatchingRobotGraph(**model_cfg)
...
def main(cfg: DictConfig):
    ...
    # Build model config (convert OmegaConf -> dict for nn.Module)
    model_cfg = OmegaConf.to_container(cfg.model, resolve=True)
    train_cfg = cfg.train

    module = FlowMatchingGraspModule(model_cfg, train_cfg)

    # Optional: load pretrained diffusion weights for transfer learning
