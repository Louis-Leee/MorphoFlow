"""
Flow Matching V4 training script with PyTorch Lightning + Hydra.

Uses FlowMatchingV4 with proper SE(3) geodesic interpolation:
- Translation: linear interpolation
- Rotation: SO(3) geodesic interpolation via log/exp map
- Euler ODE integration on SE(3) manifold for inference

Usage:
    # Single GPU
    python train_fm_v4.py

    # Multi-GPU DDP
    python train_fm_v4.py train.gpus=4

    # Override config via Hydra CLI
    python train_fm_v4.py train.lr=2e-4 dataset.batch_size=32

    # Resume from checkpoint
    python train_fm_v4.py train.resume_from=graph_exp/fm_v4/ckpt/epoch=49.ckpt

    # Use logit_normal t scheduling
    python train_fm_v4.py model.flow_matching_config.t_schedule=logit_normal
"""
...
from model.flow_matching_v4 import FlowMatchingV4
from dataset.CMapDataset import CMapDataset, custom_collate_fn


class FlowMatchingV4Module(pl.LightningModule):
    """Lightning module wrapping FlowMatchingV4 for DDP training."""

    def __init__(self, model_cfg: dict, train_cfg):
        super().__init__()
        self.save_hyperparameters()
        self.model = FlowMatchingV4(**model_cfg)
...
@hydra.main(config_path="config", config_name="train_fm_v4", version_base=None)
def main(cfg: DictConfig):
    print("\n" + "=" * 60)
    print("FLOW MATCHING V4 TRAINING (SE3 Geodesic)")
    print("=" * 60)
    ...
    print(f"t_schedule: {cfg.model.flow_matching_config.get('t_schedule', 'linear')}")
    print(f"shift_t: {cfg.model.flow_matching_config.get('shift_t', 1.0)}")
    print("=" * 60 + "\n")
    ...
    module = FlowMatchingV4Module(model_cfg, train_cfg)
